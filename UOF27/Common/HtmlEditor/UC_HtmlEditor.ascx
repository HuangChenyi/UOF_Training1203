    ...
    <script type="text/javascript">
        ...
        Sys.Application.add_load(function () {
            ...
            $("a").on("click", function (event) {
                ...
                isMatch = re.test(decodeHref);
                isMatch2 = re2.test(decodeHref);
                isMatch3 = decodeHref.indexOf("OnlineLookShow.aspx");

                if (isMatch || isMatch2 || isMatch3 > 0) {

                    if (href.indexOf('InAppBrowser') > 0)
                        return;

                    if (href.indexOf('DMS') > 0)
                        $(this).attr("href", href + '?InAppBrowser=1');
                    else if (href.indexOf('OnlineLookShow.aspx') > 0)
                        $(this).attr("href", href + '&InAppBrowser=1');
                }
            });
...
<div id="<%=ClientID %>" class="<%=ClientID %>" style="min-width: 500px;">
    ...
    <table style="margin: 1px; width: auto;" runat="server" id="tbFileList" class="NormalPopTable"></table>
