...
<script id="redirectjs" type="text/javascript">
    ...
    function cvReleaseDateTimeLaterThanCurrentDateTimeValidation(sender, args) {

        var rbDateRelease = $("#<%=rbDateRelease.ClientID%>");
        var rdpReleaseDateTime = $find("<%= rdpReleaseDateTime.ClientID %>");
        var releaseDateTime = rdpReleaseDateTime.get_dateInput().get_value();
        var nowDateTimeFromDb = $uof.pageMethod.sync("GetNowDateTimeFromDb", []);

        var releaseDateTimeToDT = new Date(releaseDateTime);
        var nowDateTimeFromDbToDT = new Date(nowDateTimeFromDb);

        if (rbDateRelease.is(":checked") && nowDateTimeFromDbToDT > releaseDateTimeToDT)
            args.IsValid = false;
        else
            args.IsValid = true;
    }
            ...
            <td style="height: 162px" valign="top">
                <table width="100%" class="PopTable" cellspacing="1">
                    ...
                    <tr>
                        ...
                        <td colspan="3">&nbsp;
                            <asp:TextBox ID="txtVersionID" runat="server" Enabled="False" Style="text-align: right" Width="34px" meta:resourcekey="txtVersionIDResource1"></asp:TextBox>
                    ...
                    <tr>
                        ...
                        <td colspan="3" valign="top">
                            ...
                            <asp:CustomValidator ID="CustomValidator4" runat="server" ErrorMessage="表單欄位不得為空"
                                Display="Dynamic" meta:resourcekey="CustomValidator4Resource1"></asp:CustomValidator>
                            <asp:CustomValidator ID="cvFormChooseFieldFormNotExist" runat="server" 
                                Display="Dynamic" meta:resourcekey="cvFormChooseFieldFormNotExistResource1"></asp:CustomValidator>
                            <asp:CustomValidator ID="cvReleaseDateTimeLaterThanCurrentDateTime" runat="server" ErrorMessage="指定發佈時間需晚於目前系統時間" 
                                Display="Dynamic" meta:resourcekey="cvReleaseDateTimeLaterThanCurrentDateTimeResource1" ClientValidationFunction="cvReleaseDateTimeLaterThanCurrentDateTimeValidation" >
                            </asp:CustomValidator>
                        </td>
                    </tr>
                </table>
