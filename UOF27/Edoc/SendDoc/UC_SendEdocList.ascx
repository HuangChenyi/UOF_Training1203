...
<script type="text/javascript">
    ...
    var lblUpdateSendDateTitle = '<%=lblUpdateSendDateTitle.ClientID%>';
    var lblSelectUpdateSendDate = '<%=lblSelectUpdateSendDate.ClientID%>';
    var lblUpdateSettingCheck = '<%=lblUpdateSettingCheck.ClientID%>';
    ...
    function rtbSendDocClientClick(sender, args) {
        ...
        if (args.get_item().get_value() === "UpdateSendDate") {
            // 判斷是否有公文被勾選
            if ($('#' + grdSendEdocListChecked).length == 0) {

                ShowAlertModal($('#' + lblUpdateSendDateTitle).text(), $('#' + lblOk).text(), $('#' + lblSelectUpdateSendDate).text());

                args.set_cancel(true);

                return;
            } else {                
                var allowUpdate = $uof.pageMethod.syncUc("/Edoc/SendDoc/UC_SendEdocList.ascx", "CheckUpdateSetting",[]);
                if (allowUpdate == "false") {
                    ShowAlertModal($('#' + lblUpdateSendDateTitle).text(), $('#' + lblOk).text(), $('#' + lblUpdateSettingCheck).text());
                    return;
                }
                var allGridDocId = GetGridDocIdList();
                $uof.dialog.open2("~/Edoc/SendDoc/SendEdocUpdateDialog.aspx", $('#' + lblUpdateSendDateTitle).text(), $('#' + lblUpdateSendDateTitle).text(), 600, 400,
                    result, { 'sendDocId': allGridDocId });
                args.set_cancel(true);
            }
        }
    }
    ...
    function result(returnValue) {
        if (typeof (returnValue) != 'undefined' || returnValue != null) {
            ShowAlertModal($('#' + lblUpdateSendDateTitle).text(), $('#' + lblOk).text(), returnValue);

            var sendToolbar = $find(rtbSendDoc);
            var queryBtn = sendToolbar.findItemByValue("Query");
            queryBtn.click();
            return true;
        }
        else {
        ...
        <telerik:RadMultiPage ID="rmpSend" runat="server" SelectedIndex="0">
            <telerik:RadPageView ID="rpvSend" Selected="True" runat="server">
                <telerik:RadToolBar ID="rtbSendDoc" runat="server" OnButtonClick="rtbSendDoc_ButtonClick" OnClientButtonClicking="rtbSendDocClientClick"
                    ...
                    <Items>
                        <telerik:RadToolBarButton Text="變更發文日期" Value="UpdateSendDate" CheckedImageUrl="~/Common/Images/Edoc/icon_m235.png"
                            DisabledImageUrl="~/Common/Images/Edoc/icon_m235.png" HoveredImageUrl="~/Common/Images/Edoc/icon_m235.png"
                            ImageUrl="~/Common/Images/Edoc/icon_m235.png" runat="server" Visible="false">
                        </telerik:RadToolBarButton>
                        <telerik:RadToolBarButton runat="server" IsSeparator="True" Value="sUpdateSendDate" Visible="false"/>
                ...
                <Ede:Grid ID="grdSendEdocList" runat="server" Width="100%" DataKeyNames="SEND_DOC_ID"
                    ...
                    <Columns>
                        ...
                        <asp:TemplateField HeaderText="操作">
                            <ItemTemplate>
                                <asp:LinkButton ID="lbtnDetail" Text="公文資訊" runat="server"></asp:LinkButton>
                                <asp:LinkButton ID="lbtUpdateRecord" Text="異動記錄" runat="server" Visible="false"></asp:LinkButton>
                            </ItemTemplate>
                            <HeaderStyle Wrap="False" />
                            <ItemStyle Wrap="False" HorizontalAlign="Center" />
                        </asp:TemplateField>
...
<asp:Label ID="lblUpdateRecord" runat="server" Text="異動記錄" Visible="False"></asp:Label>
...
<asp:Label ID="lblUpdateSendDateTitle" runat="server" Text="變更發文日期" CssClass="cssHidden"></asp:Label>
<asp:Label ID="lblSelectUpdateSendDate" runat="server" Text="請選擇要變更發文日期的公文" CssClass="cssHidden"></asp:Label>
<asp:Label ID="lblUpdateSettingCheck" runat="server" Text="未啟用「電子公文→基本設定」中【允許機關管理員變更公文發文日期】的設定，請洽電子公文管理員。" CssClass="cssHidden"></asp:Label>
