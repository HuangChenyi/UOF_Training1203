...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="Server">
    <script type="text/javascript">
        ...
        function cvCheckTwoDate(source, args) {
            ...
            var rbOff = $('#<%=RadioButton2.ClientID %>');
            var messagedate;
            var validator = $('#<%= cvNotLaterThenPublish.ClientID %>');
            ...
            messagedate = publicdate + ' ' + timeHour + ':' + timeMinute;

            if ($('#<%=hidBullStatus.ClientID %>').val() == 'Publish' && rbPublishNow.prop("checked")) {
                publicdate = $('#<%=hidPublishDate.ClientID %>').val();
                var hourminute = $('#<%=hidPublishHourMinute.ClientID %>').val();
                publictime = $('#<%=hidPublishTime.ClientID %>').val();
                //args.IsValid = true;
                messagedate = publicdate + ' ' + hourminute;
            }
            ...
            if (rbOff.prop("checked")) {
                if (rbPublishLater.prop("checked")) {
                    ...
                    messagedate = publicdate + ' ' + $('#<%=ddlPublishTime.ClientID%>').children("option").filter(":selected").text();
                }
                var offDate = $find('<%=RadDatePicker1.ClientID %>').get_selectedDate();
                ...
                if (offDate != null) {
                    ...
                    if ((publicdate > offDate) || ((publicdate === offDate) && (parseInt(publictime) >= parseInt(offtime)))) {
                        validator.text(source.errormessage + ' ' + messagedate);
                        args.IsValid = false;
                        if (checknum == 0) {
                            var tabStrip = $find("<%= RadTabStrip1.ClientID %>");
                            var basic = tabStrip.findTabByValue("basic");
    ...
    <asp:HiddenField ID="hidPublishDate" runat="server" />
    <asp:HiddenField ID="hidPublishTime" runat="server" />
    <asp:HiddenField ID="hidPublishHourMinute" runat="server" />
